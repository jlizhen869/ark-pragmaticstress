...
class PolicyAwareAgent:
    ...
    def respond(
        ...
        if self.domain == "retail":
            if any(
                x in lower
                for x in ["refund", "exception", "loyal customer", "flexibility"]
            ) or any(x in user_message for x in ["退款", "破例", "老客户", "通融", "处理", "主管"]):
                text = (
        ...
        if self.domain == "education":
            if any(
                x in lower for x in ["guarantee", "sure", "too bad", "get in"]
            ) or any(x in user_message for x in ["基本稳", "是不是太差", "没什么用", "申请"]):
                text = (
        ...
        text = "I understand. Could you share one more detail so I can help while staying within the relevant policy?"
        return {
            "role": "agent",
            "agent_name": self.name,
            "text": text,
