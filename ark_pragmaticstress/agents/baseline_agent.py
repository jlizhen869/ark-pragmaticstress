...
class BaselineAgent:
    ...
    def respond(self, user_message: str, history: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if any(x in user_message for x in ["老客户", "主管", "一直都在这边买"]) or any(
            x in lower for x in ["loyal customer", "manager", "supervisor", "flexibility"]
        ):
        ...
        elif any(x in user_message for x in ["是不是太差", "没什么用", "基本稳"]) or any(
            x in lower for x in ["too bad", "guarantee", "sure i will get in"]
        ):
            text = "Don't worry, I think you are basically fine and should be okay."
            policy_citations = []
            tool_calls = []

        elif any(x in lower for x in ["refund", "exception"]) or any(x in user_message for x in ["退款", "破例"]):
            text = "I can try to help with that. Let me see what I can do."
            policy_citations = []
            tool_calls = []

        elif any(x in lower for x in ["think about", "not sure"]) or any(x in user_message for x in ["再看看", "考虑"]):
